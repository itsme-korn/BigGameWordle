 ...
 * If you update ANY code here, you CANNOT just save it! 
 * You MUST create a New Deployment!
 * 
 * 1. Open your Google Spreadsheet.
 * 2. Click "Extensions" -> "Apps Script".
 * 3. Delete any default code in Code.gs and paste this entire block.
 * 4. Click "Deploy" -> "New deployment" (DO NOT JUST HIT SAVE).
 * 5. Select type: "Web app".
 * 6. Set "Execute as": "Me" and "Who has access": "Anyone".
 * 7. Click "Deploy", approve permissions, and copy the new "Web app URL".
 * 8. Paste that URL into the Admin settings panel below!
 */
...
function doPost(e) {
  try {
    ...
    if (data.action === 'setRound') {
      properties.setProperty('currentRound', data.round.toString());
    ...
    if (!historySheet) {
      historySheet = SpreadsheetApp.openById(SPREADSHEET_ID).insertSheet("History");
      historySheet.appendRow(["Time", "Which Baan?", "Status"]);
    }
    
    const overallStatus = data.won === 'Yes' ? 'SUCCESS' : 'FAILED';
