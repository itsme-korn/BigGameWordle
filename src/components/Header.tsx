...
export function Header({ userData, currentRound }: HeaderProps) {
  return (
    <header className="fixed top-0 left-0 right-0 h-16 w-full bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-700 flex items-center justify-between px-4 sm:px-6 text-white shadow-md z-10">
      ...
      <div className="flex items-center space-x-2 sm:space-x-4">
        {currentRound !== undefined && (
          <div className="bg-white/20 px-2 sm:px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest border border-white/30 shadow-sm backdrop-blur-md">
            Round {currentRound}
          </div>
        )}
